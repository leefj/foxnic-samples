...
function ListPage() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					...
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("regionLocation",data.arr,data.change,data.isAdd);
				},1);
			},
	...
	function bindButtonEvent() {
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('订单地址')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('订单地址')+fox.translate('吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				top.layer.confirm(fox.translate('确定删除此')+fox.translate('订单地址')+fox.translate('吗？'), function (i) {
					top.layer.close(i);
