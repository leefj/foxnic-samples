...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('webfull-example-order-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('webfull-example-order-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'webfull-example-order-form-data-win');
			if(area==null) return;
			admin.putTempData('webfull-example-order-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("addressId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			searchField: "address", //请自行调整用于搜索的字段名称
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					opts.push({data:data[i],name:data[i].address,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)});
				}
				return opts;
			}
		});
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('webfull-example-order-form-data');
		}
		...
		if (hasData) {
			...
			//设置  收件地址 设置下拉框勾选
			fox.setSelectValue4QueryApi("#addressId",formData.address);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('webfull-example-order-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('webfull-example-order-form-data-win',this); });
